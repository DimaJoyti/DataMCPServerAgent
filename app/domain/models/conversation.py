...
class ConversationSummary(BaseValueObject):
    ...
    overall_sentiment: Optional[SentimentType] = Field(
...
class ConversationMetrics(BaseEntity):
    ...
    escalation_triggers: List[str] = Field(
        default_factory=list, description="Triggered escalations"
    )
...
class LiveConversation(AggregateRoot):
    ...
    channel_metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Channel-specific data"
    )
