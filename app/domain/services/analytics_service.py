...
from app.domain.models.conversation import LiveConversation, ConversationMessage
...
class AnalyticsService(DomainService, LoggerMixin):
    ...
    async def collect_conversation_metrics(
        ...
        await self.publish_event(AnalyticsEvent(
    ...
    async def collect_agent_performance(
        ...
        performance.completed_conversations = len([
            ca for ca in conversation_analytics 
            if ca.resolution_status in ["resolved", "partially_resolved"]
        ])
        
        # Quality metrics
        satisfactions = [ca.user_satisfaction for ca in conversation_analytics if ca.user_satisfaction]
        if satisfactions:
        ...
        performance.resolution_rate = (
            ...
            if performance.total_conversations > 0 else 0.0
        )
        ...
        # Performance metrics
        response_times = [ca.avg_response_time_ms for ca in conversation_analytics if ca.avg_response_time_ms > 0]
        if response_times:
    ...
    async def get_analytics_dashboard_data(
        ...
        # Get recent alerts
        dashboard_data["alerts"] = await self._get_recent_alerts(scope, scope_id, start_time, end_time)
    ...
    async def _check_metric_threshold(
        ...
        if cooldown_key in self._alert_cooldowns:
            if datetime.now(timezone.utc) - self._alert_cooldowns[cooldown_key] < timedelta(minutes=15):
        ...
        if severity:
            ...
            self.logger.warning(f"Performance alert: {alert_type} for {scope_id} - {value} vs {threshold_value}")
