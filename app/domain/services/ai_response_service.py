...
from app.domain.models.conversation import ConversationMessage, LiveConversation, MessageAnalysis
...
class AIResponseService(DomainService, LoggerMixin):
    ...
    async def generate_response(
        ...
        try:
            ...
            self.logger.info(f"AI response generated in {generation_time_ms}ms using {provider.name}")
...
"""
    ...
    def _post_process_response(self, response: str, agent: BrandAgent) -> str:
        ...
        personality = agent.personality
        if personality.custom_phrases and not any(phrase in response for phrase in personality.custom_phrases):
    ...
    def _check_personality_match(self, response: str, personality: BrandPersonality) -> float:
        ...
        # Check tone
        if personality.tone == "friendly" and any(word in response.lower() for word in ["happy", "glad", "pleased"]):
            score += 0.2
        elif personality.tone == "professional" and not any(word in response.lower() for word in ["hey", "yo", "sup"]):
            score += 0.2
        ...
        # Check formality
        if personality.formality_level == "formal" and not any(word in response.lower() for word in ["gonna", "wanna", "yeah"]):
            score += 0.1
