...
class KnowledgeIntegrationService(DomainService, LoggerMixin):
    ...
    async def search_relevant_knowledge(
        ...
        scored_items.sort(key=lambda x: (x.relevance_score, x.metadata.get("priority", 1)), reverse=True)
    ...
    async def get_contextual_knowledge(
        ...
        knowledge_types = self._get_relevant_knowledge_types(message.analysis.intent if message.analysis else None)
    ...
    def _get_intent_knowledge_boost(self, intent: IntentType, knowledge_type: KnowledgeType) -> float:
    ...
    def _get_relevant_knowledge_types(self, intent: Optional[IntentType]) -> Optional[List[KnowledgeType]]:
        ...
        intent_type_mapping = {
            ...
            IntentType.SUPPORT: [KnowledgeType.FAQ, KnowledgeType.PROCEDURES, KnowledgeType.POLICIES],
