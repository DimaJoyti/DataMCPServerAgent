...
class ChatWebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, conversation_id: str, session_token: str):
        ...
        if status:
            await self.send_message(websocket, {"type": "conversation_status", "data": status})
    ...
    async def handle_user_message(
        self, websocket: WebSocket, conversation_id: str, message: WebSocketMessage
    ):
        """Handle user message."""
        try:
            data = message.data
            ...
            await self.broadcast_to_conversation(
                conversation_id, {"type": "agent_typing", "data": {"is_typing": True}}
            )
            ...
            await self.broadcast_to_conversation(
                conversation_id, {"type": "agent_typing", "data": {"is_typing": False}}
            )
    ...
    async def handle_typing_indicator(
        self, conversation_id: str, message: WebSocketMessage, is_typing: bool
    ):
        ...
        await self.broadcast_to_conversation(
            ...
            {
                "type": "user_typing",
                "data": {"is_typing": is_typing, "user_id": message.data.get("user_id")},
            },
        )
    ...
    async def handle_message_read(self, conversation_id: str, message: WebSocketMessage):
        ...
        if message_id:
            ...
            await self.broadcast_to_conversation(
                ...
                {
                    "type": "message_read",
                    "data": {"message_id": message_id, "read_at": datetime.now().isoformat()},
                },
            )
    ...
    async def send_system_message(
        self, conversation_id: str, message: str, message_type: str = "info"
    ):
