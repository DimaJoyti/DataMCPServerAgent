...
from .data_types import OHLCV, MarketDataMessage, MarketDataSnapshot, OrderBook, Quote, Tick, Trade
...
class TickProcessor:
    ...
    def __init__(
        self, name: str = "TickProcessor", max_symbols: int = 10000, tick_buffer_size: int = 100000
    ):
        self.name = name
        ...
        self.ohlcv_data: Dict[str, Dict[str, OHLCV]] = defaultdict(
            dict
        ...
        self.quality_metrics: Dict[str, Dict] = defaultdict(
            lambda: {
                "total_messages": 0,
                "out_of_sequence": 0,
    ...
    async def _update_snapshot_from_tick(self, tick: Tick) -> None:
        ...
        if symbol not in self.snapshots:
            self.snapshots[symbol] = MarketDataSnapshot(
                symbol=symbol, timestamp=tick.timestamp, exchange=tick.exchange
            )
    ...
    async def _update_snapshot_from_quote(self, quote: Quote) -> None:
        ...
        if symbol not in self.snapshots:
            self.snapshots[symbol] = MarketDataSnapshot(
                symbol=symbol, timestamp=quote.timestamp, exchange=quote.exchange
            )
    ...
    async def _update_snapshot_from_trade(self, trade: Trade) -> None:
        ...
        if symbol not in self.snapshots:
            self.snapshots[symbol] = MarketDataSnapshot(
                symbol=symbol, timestamp=trade.timestamp, exchange=trade.exchange
            )
    ...
    async def _process_order_book(self, order_book: OrderBook) -> None:
        ...
        if symbol not in self.snapshots:
            self.snapshots[symbol] = MarketDataSnapshot(
                symbol=symbol, timestamp=order_book.timestamp, exchange=order_book.exchange
            )
    ...
    def get_processing_stats(self) -> Dict[str, Any]:
        ...
        avg_latency = (
            sum(self.processing_latency_us) / len(self.processing_latency_us)
            if self.processing_latency_us
            else 0
        )
