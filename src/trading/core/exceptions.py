...
class OrderValidationError(TradingSystemError):
    ...
    def __init__(
        self, message: str, order_id: Optional[str] = None, validation_errors: Optional[Dict] = None
    ):
...
class InsufficientFundsError(TradingSystemError):
    ...
    def __init__(
        self, message: str, required_amount: float, available_amount: float, currency: str
    ):
...
def classify_error(error: Exception) -> Dict[str, str]:
    ...
    classification = error_mapping.get(
        error_type, {"severity": ErrorSeverity.MEDIUM, "category": ErrorCategory.TECHNICAL}
    )
