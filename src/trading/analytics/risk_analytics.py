...
from ..core.base_models import BasePosition, BaseTrade
...
class RiskAnalytics:
    ...
    async def _calculate_portfolio_var(self) -> None:
        ...
        if len(returns) > 1:
            ...
            portfolio_var["volatility"] = {
                "daily": volatility,
                "annualized": annualized_vol
            }
        
        self.var_metrics["PORTFOLIO"] = portfolio_var
        self.portfolio_var_history.append((datetime.utcnow(), portfolio_var.get("var_95", {}).get("amount", 0)))
    ...
    async def _calculate_concentration_metrics(self) -> None:
        ...
        if largest_position > self.concentration_limit:
            await self._trigger_risk_alert("CONCENTRATION_LIMIT_EXCEEDED", {
                "type": "position",
        ...
        while self.is_running:
            try:
                ...
                if var_95 > float(self.var_limit):
                    await self._trigger_risk_alert("VAR_LIMIT_EXCEEDED", {
                        "var_amount": var_95,
                ...
                for symbol, position in self.positions.items():
                    if symbol in self.position_limits:
                        ...
                        if position_size > limit:
                            await self._trigger_risk_alert("POSITION_LIMIT_EXCEEDED", {
                                "symbol": symbol,
