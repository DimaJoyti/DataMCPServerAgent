...
from ..core.base_models import BaseOrder, BaseTrade, BasePosition
from ..core.enums import OrderSide, OrderStatus
from ..market_data.data_types import MarketDataSnapshot, Trade, Quote
...
class RealTimeAnalytics:
    ...
    def __init__(
        self,
        name: str = "RealTimeAnalytics",
        calculation_frequency_ms: int = 100
    ):
        self.name = name
    ...
    async def _recalculate_pnl(self, symbol: str) -> None:
        ...
        try:
            ...
            for handler in self.pnl_update_handlers:
                try:
                    await handler(symbol, total, self.realized_pnl[symbol], self.unrealized_pnl[symbol])
    ...
    async def _calculate_risk_metrics(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                # Check risk limits
                if portfolio_var and abs(portfolio_var) > float(self.portfolio_value) * 0.02:  # 2% VaR limit
                    for handler in self.risk_alert_handlers:
                        try:
                            await handler("VAR_LIMIT_EXCEEDED", {
                                "var": portfolio_var,
    ...
    async def _monitor_performance(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                avg_latency = sum(self.calculation_latency_us) / len(self.calculation_latency_us) if self.calculation_latency_us else 0
    ...
    def get_portfolio_pnl(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "portfolio_value": float(self.portfolio_value),
            "return_pct": float(self.portfolio_pnl / self.portfolio_value * 100) if self.portfolio_value > 0 else 0.0
        }
    ...
    def get_portfolio_metrics(self) -> Dict[str, Any]:
        ...
        if returns:
            ...
            metrics["returns"] = {
                ...
                "mean": statistics.mean(returns),
                "std": statistics.stdev(returns) if len(returns) > 1 else 0.0,
                "sharpe": statistics.mean(returns) / statistics.stdev(returns) * (252 ** 0.5) if len(returns) > 1 and statistics.stdev(returns) > 0 else 0.0
            }
    ...
    def get_analytics_performance(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "calculations_per_second": self.calculation_count / max(1, self.calculation_frequency_ms / 1000),
