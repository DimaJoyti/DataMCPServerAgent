...
from ..market_data.data_types import Quote, Trade, OrderBook, MarketDataSnapshot
...
class MarketMicrostructureAnalyzer:
    ...
    def __init__(
        ...
        self.trade_classification: Dict[str, Dict] = defaultdict(lambda: {
            "aggressive_buy": 0,
            "aggressive_sell": 0,
            "passive_buy": 0,
            "passive_sell": 0
        ...
        if spreads:
            ...
            if spread_bps:
                metrics.update({
                    ...
                    "std_spread_bps": statistics.stdev(spread_bps) if len(spread_bps) > 1 else 0.0
                })
    ...
    async def _calculate_liquidity_metrics(self, symbol: str) -> None:
        ...
        if len(books) >= 2:
            ...
            if time_diff > 0:
                ...
                if (latest_book.best_bid and prev_book.best_bid and 
                    latest_book.best_ask and prev_book.best_ask):
    ...
    async def _calculate_market_impact(self, symbol: str) -> None:
        ...
        if temporary_impacts:
            metrics.update({
                ...
                "temporary_impact_std": statistics.stdev(temporary_impacts) if len(temporary_impacts) > 1 else 0.0
            })
        ...
        if permanent_impacts:
            metrics.update({
                ...
                "permanent_impact_std": statistics.stdev(permanent_impacts) if len(permanent_impacts) > 1 else 0.0
            })
        ...
        if recent_trades and temporary_impacts:
            ...
            if volumes:
                ...
                metrics["impact_per_volume"] = avg_temp_impact / avg_volume if avg_volume > 0 else 0.0
    ...
    def get_market_quality_score(self, symbol: str) -> Optional[float]:
        ...
        spread_score = max(0, 1 - spread_metrics.get("mean_spread_bps", 100) / 100)  # Normalize to 100 bps
        liquidity_score = min(1, liquidity_metrics.get("total_volume_L5", 0) / 10000)  # Normalize to 10k shares
    ...
    def get_analyzer_performance(self) -> Dict[str, Any]:
        ...
        avg_latency = sum(self.analysis_latency_us) / len(self.analysis_latency_us) if self.analysis_latency_us else 0
