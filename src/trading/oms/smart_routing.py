...
from ..core.enums import Exchange, OrderSide, OrderType
...
class SmartOrderRouter:
    ...
    def _select_routing_strategy(
        self, order: BaseOrder, venue_quotes: Dict[Exchange, VenueQuote]
    ) -> str:
    ...
    async def _route_best_price(
        ...
        for exchange, quote in venue_quotes.items():
            ...
            if price is not None and (
                best_price is None
                or (order.side == OrderSide.BUY and price < best_price)
                or (order.side == OrderSide.SELL and price > best_price)
            ):
                best_price = price
                best_venue = exchange
    ...
    async def _route_liquidity_seeking(
        self, order: BaseOrder, venue_quotes: Dict[Exchange, VenueQuote]
    ) -> RoutingDecision:
    ...
    def _calculate_venue_score(
        self, order: BaseOrder, exchange: Exchange, quote: VenueQuote
    ) -> float:
    ...
    def _calculate_execution_cost(
        self, order: BaseOrder, exchange: Exchange, price: Decimal
    ) -> Decimal:
    ...
    def _calculate_execution_cost_for_quantity(
        self, order: BaseOrder, exchange: Exchange, price: Decimal, quantity: Decimal
    ) -> Decimal:
