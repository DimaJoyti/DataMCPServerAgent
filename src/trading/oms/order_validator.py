...
from ..core.enums import ASSET_CLASS_CONFIGS, EXCHANGE_CONFIGS, AssetClass, Exchange, OrderSide, OrderType
...
class OrderValidator:
        ...
        if order.price is not None and order.stop_price is not None:
            if order.side == OrderSide.BUY:
                if order.stop_price <= order.price:
                    result.add_error("Stop price should be above limit price for buy stop-limit orders")
            else:
                if order.stop_price >= order.price:
                    result.add_error("Stop price should be below limit price for sell stop-limit orders")
    ...
    def _validate_order_type(self, order: BaseOrder, result: ValidationResult) -> None:
        ...
        if order.order_type in [OrderType.TWAP, OrderType.VWAP, OrderType.IMPLEMENTATION_SHORTFALL]:
            if order.quantity < Decimal('1000'):
                result.add_warning("Small quantity for algorithmic order - consider direct execution")
    ...
    def _validate_asset_class(self, order: BaseOrder, result: ValidationResult) -> None:
        ...
        if asset_class in ASSET_CLASS_CONFIGS:
            ...
            if order.side in [OrderSide.SHORT, OrderSide.SELL] and not config.get('short_selling_allowed', True):
