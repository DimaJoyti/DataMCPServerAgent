...
from ..market_data.data_types import Quote, Trade, OHLCV, MarketDataSnapshot
            ...
            if 'bid_size' in df.columns and 'ask_size' in df.columns:
                ...
                for period in [10, 20]:
                    if len(imbalance) >= period:
                        features[f'imbalance_mean_{period}'] = imbalance.rolling(window=period).mean()
            ...
            if 'size' in df.columns:
                ...
                for period in [10, 20, 50]:
                    if len(volume) >= period:
                        ...
                        features[f'volume_ratio_{period}'] = volume / volume.rolling(window=period).mean()
