...
from ..market_data.data_types import MarketDataSnapshot, OHLCV, Quote, Trade
...
class MLEngine:
    ...
    async def update_market_data(self, symbol: str, data: Union[Quote, Trade, OHLCV, MarketDataSnapshot]) -> None:
    ...
    async def predict(
        ...
        try:
            ...
            features = await self._get_features_for_prediction(symbol, model_info['feature_columns'])
            if features is None:
            ...
            self.logger.debug(f"Generated prediction for {symbol}: {prediction} (confidence: {confidence:.2f})")
            return result
    ...
    async def _get_features_for_prediction(self, symbol: str, feature_columns: List[str]) -> Optional[pd.Series]:
    ...
    async def _calculate_model_performance(self, model_id: str) -> None:
        ...
        try:
            ...
            performance = {
                ...
                'avg_latency_us': sum(self.inference_latency_us) / len(self.inference_latency_us) if self.inference_latency_us else 0
            }
    ...
    def get_engine_stats(self) -> Dict[str, Any]:
        ...
        avg_latency = sum(self.inference_latency_us) / len(self.inference_latency_us) if self.inference_latency_us else 0
