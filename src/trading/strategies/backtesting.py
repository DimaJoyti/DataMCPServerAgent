...
class BacktestingEngine:
        ...
        try:
            ...
            for symbol, df in historical_data.items():
                if 'timestamp' in df.columns:
                    ...
                    all_timestamps.update(timestamps[(timestamps >= start_date) & (timestamps <= end_date)])
            ...
            for symbol in strategy.symbols:
                if symbol in historical_data:
                    ...
                    if 'timestamp' in df.columns:
                        ...
                        current_data = df.iloc[:df.index.get_loc(timestamp) + 1] if timestamp in df.index else df
                    ...
                    if len(current_data) > 0:
                        ...
                        if signal:
                            await self._process_signal(strategy, symbol, signal, market_data, timestamp)
    ...
    def _should_exit_position(self, signal: StrategySignalData, position: Dict[str, Any]) -> bool:
        ...
        if position_side == OrderSide.BUY:
            return signal.signal in [StrategySignal.SELL, StrategySignal.STRONG_SELL, StrategySignal.WEAK_SELL]
        else:
            return signal.signal in [StrategySignal.BUY, StrategySignal.STRONG_BUY, StrategySignal.WEAK_BUY]
    ...
    async def _open_position(
        ...
        try:
            ...
            if signal.signal in [StrategySignal.BUY, StrategySignal.STRONG_BUY, StrategySignal.WEAK_BUY]:
                side = OrderSide.BUY
            else:
                side = OrderSide.SELL
    ...
    async def _close_all_positions(self, historical_data: Dict[str, pd.DataFrame], final_timestamp: datetime) -> None:
    ...
    def _calculate_current_equity(self, historical_data: Dict[str, pd.DataFrame], timestamp: datetime) -> Decimal:
