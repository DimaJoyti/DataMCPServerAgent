    ...
    async def generate_signal(
        ...
        try:
            ...
            bb_data = TechnicalIndicators.bollinger_bands(
                df["close"], self.parameters["bb_period"], self.parameters["bb_std_dev"]
            )
            ...
            # Generate signals
            if (
                lower_distance <= self.parameters["oversold_threshold"]
                ...
                upper_distance <= self.parameters["overbought_threshold"]
                and rsi_confirmation
                and volume_confirmation
            ):
    ...
    async def generate_signal(
        ...
        try:
            ...
            volatility = (
                df["close"]
                .pct_change()
                .rolling(window=self.parameters["lookback_period"])
                .std()
                .iloc[-1]
            )
            ...
            return StrategySignalData(
                ...
                indicators={
                    ...
                    "rolling_mean": df["close"]
                    .rolling(window=self.parameters["lookback_period"])
                    .mean()
                    .iloc[-1],
                    "rolling_std": df["close"]
                    .rolling(window=self.parameters["lookback_period"])
                    .std()
                    .iloc[-1],
                },
...
class RSIMeanReversionStrategy(EnhancedBaseStrategy):
    ...
    async def generate_signal(
        ...
        try:
            ...
            if (
                df is None
                or len(df)
                < self.parameters["rsi_period"] + self.parameters["divergence_lookback"] + 10
            ):
                return None
            ...
            if current_rsi <= self.parameters["extreme_oversold"]:
                signal = StrategySignal.STRONG_BUY
                strength = (self.parameters["extreme_oversold"] - current_rsi) / self.parameters[
                    "extreme_oversold"
                ]
                confidence = 0.9
                ...
                signal = StrategySignal.STRONG_SELL
                strength = (current_rsi - self.parameters["extreme_overbought"]) / (
                    100 - self.parameters["extreme_overbought"]
                )
                confidence = 0.9
            ...
            # Regular oversold/overbought levels
                ...
                signal = StrategySignal.SELL
                strength = (current_rsi - self.parameters["overbought"]) / (
    ...
    def _detect_bullish_divergence(self, df: pd.DataFrame, rsi: pd.Series) -> bool:
        ...
        try:
            ...
            return (
                price_recent < price_prev_low
                and rsi_recent > rsi_prev_low
                and rsi_recent < self.parameters["oversold"]
            )
        except:
            return False
    ...
    def _detect_bearish_divergence(self, df: pd.DataFrame, rsi: pd.Series) -> bool:
        ...
        try:
            ...
            return (
                price_recent > price_prev_high
                and rsi_recent < rsi_prev_high
                and rsi_recent > self.parameters["overbought"]
            )
        except:
            return False
    ...
    async def calculate_position_size(self, symbol: str, signal: StrategySignalData) -> Decimal:
        ...
        if signal.indicators.get("bullish_divergence") or signal.indicators.get(
            "bearish_divergence"
        ):
