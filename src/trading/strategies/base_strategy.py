...
class StrategyMetrics:
    ...
    def update_metrics(self, trade_pnl: Decimal) -> None:
        ...
        if trade_pnl > 0:
            ...
            self.avg_loss = (self.avg_loss * (self.losing_trades - 1) + abs(trade_pnl)) / self.losing_trades
...
class EnhancedBaseStrategy(BaseStrategy):
    ...
    async def process_signal(self, symbol: str, signal: StrategySignalData) -> Optional[Dict[str, Any]]:
    ...
    async def _generate_order(
        ...
        side = OrderSide.BUY if signal.signal in [
            StrategySignal.STRONG_BUY, StrategySignal.BUY, StrategySignal.WEAK_BUY
        ] else OrderSide.SELL
