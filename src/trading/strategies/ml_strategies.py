...
class RandomForestStrategy(EnhancedBaseStrategy):
    ...
    def __init__(
        ...
        if not ML_AVAILABLE:
            raise ImportError(
                "Machine learning libraries not available. Install scikit-learn and tensorflow."
            )
    ...
    async def generate_signal(
        ...
        try:
            ...
            if (
                ...
                or symbol not in self.last_training
                or (datetime.now() - self.last_training[symbol]).total_seconds()
    ...
    async def _train_model(self, symbol: str) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Trained Random Forest model for {symbol} with accuracy: {accuracy:.3f}"
            )
    ...
    async def _prepare_training_data(
        ...
        try:
            ...
            df_with_indicators["high_low_ratio"] = (
            ...
            for period in [5, 10, 20]:
                df_with_indicators[f"return_{period}d"] = df_with_indicators["close"].pct_change(
                    period
                )
                df_with_indicators[f"volatility_{period}d"] = (
                    df_with_indicators["close"].pct_change().rolling(period).std()
                )
            ...
            df_with_indicators["future_return"] = (
                df_with_indicators["close"].shift(-future_periods) / df_with_indicators["close"] - 1
            )
...
class LSTMStrategy(EnhancedBaseStrategy):
    ...
    async def _train_lstm_model(self, symbol: str) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Trained LSTM model for {symbol} with validation loss: {self.model_loss[symbol]:.6f}"
            )

        except Exception as e:
            self.logger.error(f"Error training LSTM model for {symbol}: {e}")

    async def _prepare_lstm_training_data(
        self, symbol: str, df: pd.DataFrame
    ) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
