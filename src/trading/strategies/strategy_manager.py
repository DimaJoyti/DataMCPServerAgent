...
class PortfolioMetrics:
    ...
    def update_from_strategies(self, strategies: Dict[str, EnhancedBaseStrategy]) -> None:
        ...
        total_losses = sum(
            s.metrics.avg_loss * s.metrics.losing_trades for s in strategies.values()
        )
...
class StrategyManager:
    ...
    def __init__(
        ...
        # Risk management
        self.max_portfolio_drawdown = self.risk_parameters.get(
            "max_portfolio_drawdown", Decimal("0.15")
        )
        self.max_correlation_threshold = self.risk_parameters.get("max_correlation_threshold", 0.7)
        self.min_strategy_allocation = self.risk_parameters.get(
            "min_strategy_allocation", Decimal("0.05")
        )
    ...
    async def add_strategy(
        self, strategy: EnhancedBaseStrategy, allocation_percentage: float, priority: int = 1
    ) -> bool:
        ...
        try:
            ...
            total_allocation = sum(
                alloc.allocation_percentage for alloc in self.allocations.values()
            )
            if total_allocation + allocation_percentage > 1.0:
                self.logger.error(
                    f"Total allocation would exceed 100%: {total_allocation + allocation_percentage}"
                )
                return False
            ...
            self.logger.info(
                f"Added strategy {strategy.name} with {allocation_percentage*100:.1f}% allocation"
            )
            return True
    ...
    async def update_market_data(self, symbol: str, market_data: MarketData) -> None:
        ...
        try:
            ...
            self.price_history[symbol] = pd.concat([self.price_history[symbol], new_data]).tail(
                1000
            )
        ...
        try:
            for strategy in self.strategies.values():
                if (
                    ...
                    or not self.allocations[strategy.strategy_id].is_active
                ):
                    continue
    ...
    async def _rebalance_loop(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                if datetime.now() - self.last_rebalance > timedelta(
                    seconds=self.rebalance_interval
                ):
            ...
            if total_score > 0:
                for strategy_id, allocation in self.allocations.items():
                    if strategy_id in performance_scores:
                        ...
                        allocation.max_allocation = self.total_capital * Decimal(
                            str(new_percentage)
                        )
        ...
        while self.is_running:
            try:
                ...
                if self.portfolio_metrics.max_drawdown > self.max_portfolio_drawdown:
                    self.logger.warning(
                        f"Portfolio drawdown exceeded limit: {self.portfolio_metrics.max_drawdown}"
                    )
        ...
        try:
            ...
            for signal_type, count in recent_signals.items():
                if count / total_strategies > self.max_correlation_threshold:
                    self.logger.warning(
                        f"High correlation detected: {count}/{total_strategies} strategies have {signal_type} signal"
                    )
