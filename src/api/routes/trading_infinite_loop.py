...
class StrategyGenerationRequest(BaseModel):
    ...
    count: Union[int, str] = Field(default=10, description="Number of strategies to generate or 'infinite'")
    target_symbols: List[str] = Field(default=["BTC/USDT", "ETH/USDT"], description="Trading symbols to target")
    strategy_types: List[str] = Field(default=["momentum", "mean_reversion"], description="Types of strategies to generate")
    risk_tolerance: float = Field(default=0.02, ge=0.001, le=0.1, description="Risk tolerance (0.1% to 10%)")
    min_profit_threshold: float = Field(default=0.005, ge=0.001, le=0.05, description="Minimum profit threshold")
    backtest_period_days: int = Field(default=30, ge=7, le=365, description="Backtesting period in days")
    config: Optional[Dict[str, Any]] = Field(default=None, description="Additional configuration parameters")
...
class StrategyDeploymentRequest(BaseModel):
    ...
    allocation: float = Field(default=0.1, ge=0.01, le=1.0, description="Portfolio allocation (1% to 100%)")
    max_position_size: float = Field(default=0.05, ge=0.01, le=0.2, description="Maximum position size")
...
async def start_strategy_generation(
    ...
    try:
        ...
        session_id = await trading_service.start_generation(
            count=request.count,
            config=config
        )
...
async def stop_strategy_generation(session_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": True,
            "message": "Strategy generation stopped",
