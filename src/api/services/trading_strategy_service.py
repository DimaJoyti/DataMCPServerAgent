    ...
    async def get_generation_status(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        if "orchestrator" in session and hasattr(session["orchestrator"], "execution_state"):
            ...
            if execution_state:
                ...
                session["progress"] = min(execution_state.total_iterations / 100, 1.0)  # Rough progress estimate
        ...
        if filters:
            if filters.get("min_sharpe_ratio"):
                strategies = [
                    ...
                    if s.get("performance", {}).get("sharpe_ratio", 0) >= filters["min_sharpe_ratio"]
                ]
            
            if filters.get("max_drawdown"):
                strategies = [
                    s for s in strategies 
                    if abs(s.get("performance", {}).get("max_drawdown", 1)) <= filters["max_drawdown"]
                ]
