...
from ..trading_system.advanced_ml_models import ModelManager
...
class TradingStrategyOrchestrator:
    ...
    def __init__(
        ...
        self.infinite_loop = InfiniteAgenticLoopOrchestrator(
            model=model, tools=tools, config=self.config
        )
    ...
    async def generate_trading_strategies(
        ...
        results = await self.infinite_loop.execute_infinite_loop(
            spec_file=spec_file, output_dir=output_dir, count=count
        )
            ...
            try:
                ...
                if strategy_data:
                    ...
                    if self._meets_performance_criteria(performance):
                        ...
                        self.logger.info(
                            f"Added strategy {strategy_id} with Sharpe ratio: {performance.get('sharpe_ratio', 0):.2f}"
                        )
    ...
    def _evaluate_strategy_performance(self, backtest_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        performance["evaluation"] = {
            ...
            "drawdown_score": max(
                1 - abs(metrics.get("max_drawdown", 0)) / self.config.max_drawdown, 0
            ),
    ...
    def _meets_performance_criteria(self, performance: Dict[str, Any]) -> bool:
        ...
        return (
            performance.get("sharpe_ratio", 0) >= self.config.min_sharpe_ratio
            and abs(performance.get("max_drawdown", 1)) <= self.config.max_drawdown
            and performance.get("win_rate", 0) >= self.config.min_win_rate
        )
    ...
    async def get_best_strategies(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        return [
            {"strategy_id": strategy_id, **strategy_data}
