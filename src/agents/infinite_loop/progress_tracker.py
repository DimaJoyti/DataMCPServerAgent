...
class ProgressTracker:
    ...
    def get_overall_progress(self) -> Dict[str, Any]:
        ...
        if active_tasks > 0 and self.average_task_time > 0:
            remaining_time = (
                ...
                if any(task.estimated_completion for task in self.tasks.values())
                else 0
            )
        ...
        return [
            {
                ...
                "elapsed_time": (
                    (datetime.now() - task.start_time).total_seconds() if task.start_time else 0
                ),
            }
            for task in self.tasks.values()
        ]
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tasks_per_minute": (
                (self.total_tasks_completed / (self.total_execution_time / 60.0))
    ...
    def get_recent_failures(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        recent_failures = sorted(
            self.failed_tasks, key=lambda t: t.end_time or datetime.now(), reverse=True
        )[:limit]
