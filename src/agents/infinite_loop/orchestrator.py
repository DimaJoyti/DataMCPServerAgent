...
from .parallel_executor import ErrorRecoveryManager, StatePersistence
...
class InfiniteAgenticLoopOrchestrator:
    ...
    async def _analyze_specification(self, spec_file: Union[str, Path]) -> Dict[str, Any]:
        ...
        self.logger.info("Specification analysis complete:")
        ...
        self.logger.info(
            f"- Evolution pattern: {spec_analysis.get('evolution_pattern', 'unknown')}"
        )
    ...
    async def _plan_iteration_strategy(
        ...
        self.logger.info("Iteration strategy planned:")
        ...
        if count == "infinite":
            ...
            if count <= 5:
                ...
                return {
                    ...
                    "max_waves": (count + self.config.wave_size_max - 1)
                    // self.config.wave_size_max,
                    "context_monitoring": True,
                }
    ...
    async def _execute_infinite_mode(
        ...
        self.logger.info(
            f"Infinite mode completed: {results['total_iterations']} iterations across {wave_number-1} waves"
        )
        return results
    ...
    async def _execute_finite_mode(
        ...
        for wave_number in range(1, max_waves + 1):
            ...
            self.logger.info(
                f"Starting wave {wave_number}/{max_waves} with {actual_wave_size} agents"
            )
    ...
    async def _execute_wave(
        ...
        for result in wave_results:
            if result.get("success", False):
                ...
                self.execution_state.completed_iterations.append(
                    str(result.get("iteration_number"))
                )
            else:
                failed_iterations += 1
        ...
        self.execution_state.average_iteration_time = (
            (
                (
                    self.execution_state.average_iteration_time
                    * self.execution_state.total_iterations
                    + wave_time
                )
                / (self.execution_state.total_iterations + completed_iterations)
            )
            if completed_iterations > 0
            else self.execution_state.average_iteration_time
        )
        ...
        self.execution_state.success_rate = (
            (
                ...
                / (
                    len(self.execution_state.completed_iterations)
                    + len(self.execution_state.failed_iterations)
                )
            )
            if (
                len(self.execution_state.completed_iterations)
