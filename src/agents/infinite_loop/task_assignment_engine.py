...
class TaskAssignmentEngine:
    ...
    def create_task(
        ...
        if priority is None:
            priority = self._calculate_priority(
                iteration_number, complexity, directory_state
            )
    ...
    def create_batch_tasks(
        ...
        """
        self.logger.info(f"Creating batch of {count} tasks starting from iteration {starting_iteration}")
        ...
        for i in range(count):
            ...
            dimension = self._assign_innovation_dimension(
                innovation_dimensions, i, len(tasks)
            )
    ...
    def validate_task_specification(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Required fields
        required_fields = [
            "task_id", "iteration_number", "spec_analysis",
            "directory_state", "innovation_dimension", "output_dir"
        ]
