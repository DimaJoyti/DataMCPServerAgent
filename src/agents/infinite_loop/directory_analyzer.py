...
class DirectoryAnalyzer:
    ...
    async def analyze_directory(self, output_dir: Union[str, Path]) -> Dict[str, Any]:
        ...
        highest_iteration = max(
            (file_info["iteration_number"] for file_info in iteration_files), default=0
        )
        ...
        self.logger.info("Directory analysis complete:")
    ...
    async def _identify_iteration_files(
    ...
    async def _analyze_naming_patterns(
        self, iteration_files: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    async def _analyze_content_evolution(
        ...
        for i in range(1, len(iteration_files)):
            ...
            size_change_percent = (
                (size_change / prev_file["size"] * 100) if prev_file["size"] > 0 else 0
            )
    ...
    async def _identify_opportunities(
        self, iteration_files: List[Dict[str, Any]], content_evolution: List[Dict[str, Any]]
    ) -> List[str]:
        ...
        identical_pairs = [evo for evo in content_evolution if evo["is_identical"]]
        if identical_pairs:
            opportunities.append(
                f"Found {len(identical_pairs)} identical consecutive iterations - opportunity for differentiation"
            )

        # Check for large size changes
        large_changes = [evo for evo in content_evolution if abs(evo["size_change_percent"]) > 50]
        if large_changes:
            opportunities.append(
                f"Found {len(large_changes)} iterations with large size changes - opportunity for gradual evolution"
            )
        ...
        if gaps:
            opportunities.append(
                f"Found {len(gaps)} gaps in iteration sequence - opportunity to fill missing iterations"
            )
    ...
    async def _calculate_statistics(
        self, all_files: List[Dict[str, Any]], iteration_files: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
