...
VIS_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "visualizations", "seo"
)
...
class SEOVisualizationTool:
    ...
    def generate_keyword_rankings_chart(
        self,
        domain: str,
            ...
            if "history" in df.columns:
                ...
                for _, row in df.iterrows():
                    ...
                    for date, position in row["history"].items():
                        history_data.append(
                            {"keyword": keyword, "date": date, "position": position}
                        )
        ...
        chart_path = os.path.join(
            VIS_DIR, f"{domain.replace('.', '_')}_keyword_rankings_{timestamp}.png"
        )
    ...
    def generate_seo_score_comparison(
        self, domain: str, competitors: List[Dict[str, Any]], color_palette: str = "green"
    ) -> str:
        """
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        chart_path = os.path.join(
            VIS_DIR, f"{domain.replace('.', '_')}_seo_score_comparison_{timestamp}.png"
        )
        plt.savefig(chart_path, dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def generate_backlink_profile(
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        chart_path = os.path.join(
            VIS_DIR, f"{domain.replace('.', '_')}_backlink_profile_{timestamp}.png"
        )
        plt.savefig(chart_path, dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def generate_content_analysis(
        self, content: str, keywords: List[str], color_palette: str = "orange"
    ) -> str:
        """
        ...
        """
        print("Generating content analysis visualization...")
        ...
        wordcloud = WordCloud(
            width=800, height=400, background_color="white", colormap=color_palette, max_words=100
        ).generate(content)
        ...
        keyword_counts = {
            k: v for k, v in sorted(keyword_counts.items(), key=lambda item: item[1], reverse=True)
        }
    ...
    def generate_visualization(
        ...
        try:
            if data_type == "keyword_rankings":
                ...
                return self.generate_keyword_rankings_chart(
                    domain, keywords, chart_type, color_palette
                )
                ...
                return self.generate_backlink_profile(
                    domain, backlink_data, chart_type, color_palette
                )
